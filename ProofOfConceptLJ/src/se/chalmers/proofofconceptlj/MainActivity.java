...
public class MainActivity extends Activity {
	 ...
	 * 
	 * @param x
	 *            horizontal distance to sound at beginning
	 */
	public void driveByFrom(int x) {
		for (int i = -x; i < x; i++) {
			playAtCoordinate(new Vector2(i, Math.pow(i, 2)), new Human()); // follow
																			// parable
																			// y=x��
		}
	}
	...
	public void playGivenCoordinate(View view) {
		...
		Vector2 soundPos = new Vector2(Integer.parseInt(ETx.getText()
				.toString()), Integer.parseInt(ETy.getText().toString()));
	...
	public void play3D(View view) {
		// try {
		test3D();
		// } catch (Exception e) {
		// System.out.println("No 3D support on device");
		// }
	}
	...
	// The native function to be used
	public static native void test3D();

	static {
		System.loadLibrary("audio3d");
	}
